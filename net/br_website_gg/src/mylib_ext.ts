...
enum MyConst {
    JsOnelink = "https://www.domainprojectd.com/landingpage/br/static/js/onelink02_ext.js",
    JsClipboard = "https://www.domainprojectd.com/landingpage/br/static/js/clipboard.min.js",
    JsVConsole = "https://www.domainprojectd.com/landingpage/br/static/js/vconsole.min.js",
}
...
class LogUtil {
    private static logLevel: ELogLevel = ELogLevel.Error;
...
class utils {
    ...
    static getArg(key: string): string {
    ...
    public static clipboardSet(txt: string) {
        if (!txt || txt.length == 0)
            return

        return new Promise<any>(async (resolve) => {
            if (!window.itsJsClip) {
                await utils.loadScript(MyConst.JsClipboard)
                window.itsJsClip = true
            }

            await logic.createCopyButton()
    ...
    static loadScript(src: string, attrs?: object) {
    ...
    static async openDev() {
        await utils.loadScript(MyConst.JsVConsole)
        await utils.wait(0.005)
        new VConsole()
    }

    static ParseNum(num): number {
        if (!num) return
        var res = Number(num)
    ...
    static async httpReq(url: string, data: string | object, method?: "POST" | "GET", header?: Map<string, any>) {
        return new Promise<IHttpRsp>(resolve => {
    ...
    // 检测是否标准的 uuid
    static IsStdUUID(txt: string): boolean {
        return txt != null ? txt.match(/^\w{8}(-\w{4}){3}-\w{12}$/) != null : false
    }

    // 检测是否游戏 uid
    static IsGameUid(txt: string): boolean {
...
class logic {
    ...
    static async reqSrvCode(reqObj: IFbInfo) {
        ...
        let res = await utils.httpReq(url, data, "POST")
        var txt = null
        if (res.isOk) {
    ...
    static async getFbCode(pixelId: string) {
        if (window.itsfbcode) {
        ...
        var code = await logic.reqSrvCode({ fbc: _fbc, fbp: _fbp, pixelId: pixelId })
        LogUtil.D("--- req code:", code)
        window.itsfbcode = code // 缓存
        return code
    }

    static beginDownLoadApk(url: string, taEvent?: any) {
        LogUtil.A(url != null, "--- download url is null")
        taEvent = (taEvent != undefined) ? taEvent : "ta_page_upload"//"downLoad"
        var hiddenIframe = document.getElementById('hiddenIframe') as HTMLIFrameElement;
        if (!hiddenIframe) {
    ...
    static getCfg(platId: number): IPlatCfg {
    ...
    // 解析规则
    static getTgStartParam(txt: string): IDynParam {
        let param: IDynParam = {}
        ...
        try {
            ...
            let argType = utils.ParseNum(arr[0]) || ETgType.None

            // 1_5361d114-aaaa-42d3-bd06-992bba39310c_2060
            if (argType == ETgType.FbCode) {
                let fbc = arr[1]
                param.FbCode = utils.IsStdUUID(fbc) ? fbc : null
            } else if (argType == ETgType.AgentUid) {
                let aid = arr[1]
    ...
    // 落地页 使用
    static async doDownload(platId) {
        LogUtil.D("--- click download")

        var cfg = logic.getCfg(platId)
        var dstUrl = null
        if (cfg.onelink && cfg.pix) {
            var fbCode = await logic.getFbCode(cfg.pix)
            ...
            dstUrl = await logic.oneLinkGetResultUrl(msg, cfg.onelink, cfg.pid, gaid)
    ...
    static async oneLinkGetResultUrl(jsonMsg, oneLinkURL, platId, gaid) {
        if (!window.itsJsOnelink) {
            await utils.loadScript(MyConst.JsOnelink)
            window.itsJsOnelink = true
        }

        return await itsonelink.getResultUrl(jsonMsg, oneLinkURL, platId, gaid)
    }
    ...
    // 官网 使用
    static async doOfficial(platId) {
    ...
    // 官网 使用
    static async doOfficialIos(platId) {
    ...
    static showLoading() {
        ...
        // 禁用所有交互元素
        const elements: NodeListOf<HTMLElement> = document.querySelectorAll('button, input, a, img'); // 可以选择更多元素
        elements.forEach(el => {
    ...
    static hideLoading() {
        ...
        // 恢复所有交互元素
        const elements: NodeListOf<HTMLElement> = document.querySelectorAll('button, input, a, img'); // 恢复同样的元素
        elements.forEach(el => {
